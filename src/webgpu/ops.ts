...
fn main(@builtin(workgroup_id) group: vec3<u32>, @builtin(local_invocation_id) local: vec3<u32>, @builtin(subgroup_invocation_id) subgroup_lane: u32, @builtin(subgroup_id) subgroup_id: u32, @builtin(num_subgroups) num_subgroups: u32) {
  ...
  workgroupBarrier();
  let rotary_d = d % 64u;
  let angle = f32(token % params.sequence) * pow(params.theta, -f32(rotary_d) / 64.0);
  let partner = select(d + 64u, d - 64u, d >= 64u);
  let rotated = select(-values[partner], values[partner], d >= 64u);
  let result=f16(normed*cos(angle)+rotated*sin(angle));
...
fn main(@builtin(workgroup_id) group: vec3<u32>, @builtin(local_invocation_id) local: vec3<u32>, @builtin(local_invocation_index) lane: u32) {
  ...
  loop {
    ...
    if (!is_key) {
      let pos = group.y * 16u + row;
      q_tile[tile_index] = select(vec4<f16>(0.0), query[((batch * params.sequence + pos) * params.q_heads + q_head) * 32u + d4], pos < params.sequence);
    } else {
  ...
  workgroupBarrier();
  let q0 = group.y * 16u + local.y; let q1 = q0 + 8u;
  ...
  for (var d4 = 0u; d4 < 32u; d4 += 1u) {
    let a0 = q_tile[qa0 + d4]; let a1 = q_tile[qa1 + d4];
    let b0 = k_tile[kb0 + d4]; let b1 = k_tile[kb1 + d4];
...
enable f16;
struct Params { rows: u32, sequence: u32 }
@group(0) @binding(0) var<storage, read_write> scores: array<f16>;
@group(0) @binding(1) var<uniform> params: Params;
var<workgroup> values: array<f32, 128>;
@compute @workgroup_size(128)
fn main(@builtin(workgroup_id) group: vec3<u32>, @builtin(local_invocation_id) local: vec3<u32>) {
  let lane = local.x; let offset = group.x * params.sequence;
  values[lane] = select(-3.402823e38, f32(scores[offset + lane]), lane < params.sequence);
  workgroupBarrier();
  for (var stride=64u; stride>0u; stride>>=1u) { if (lane<stride) { values[lane]=max(values[lane],values[lane+stride]); } workgroupBarrier(); }
  let maximum=values[0]; let exponential=select(0.0, exp(f32(scores[offset+lane])-maximum), lane<params.sequence);
  values[lane]=exponential; workgroupBarrier();
  for (var stride=64u; stride>0u; stride>>=1u) { if (lane<stride) { values[lane]+=values[lane+stride]; } workgroupBarrier(); }
  if (lane<params.sequence) { scores[offset+lane]=f16(exponential/values[0]); }
}
`;
